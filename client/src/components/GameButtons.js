...
class GameButtons extends React.Component {
  ...
  dealNextHand = () => {
    ...
    for (let i = 0; i < 4; i++) {
      randomNumber = [
        ...randomNumber,
        Math.floor(Math.random() * usedDeck.length)
    ...
    this.props.getCurrentHand(hand).then(() => {
      const bankerTotal = this.baccaratCount(
        this.props.state.banker[0].value,
        this.props.state.banker[1].value
      );
      const playerTotal = this.baccaratCount(
        this.props.state.player[0].value,
        this.props.state.player[1].value
      );
  ...
  hitOrStay = (banker, player) => {
    ...
    //check for 6,7 for both
      ...
      if (banker < 3) {
        ...
        banker === 4 &&
        (playerHitCard.value > 1 && playerHitCard.value < 8)
      ) {
        ...
        banker === 5 &&
        (playerHitCard.value > 3 && playerHitCard.value < 8)
      ) {
        ...
        banker === 6 &&
        (playerHitCard.value === 6 || playerHitCard.value === 7)
      ) {
  ...
  render() {
    return (
      <GameWrapper>
        <button className="ui blue button" onClick={this.newShoe}>
          New Shoe
        </button>
        <button className="ui green button" onClick={this.newOrNextHand}>
          {this.props.deckLength < 60
            ? "Last Hand! New Game."
            : "Deal Next Hand"}
        </button>
      </GameWrapper>
